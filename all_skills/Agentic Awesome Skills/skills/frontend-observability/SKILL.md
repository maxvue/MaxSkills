...
description: "A portable, framework-agnostic field-side observability system for any React or React Native app. Establishes one typed event taxonomy (canonical event-name constants, never inline strings), a best-effort non-blocking provider fan-out so a failing or absent analytics provider can never... Use when developing, optimizing, debugging, or configuring frontend observability in production workfl..."
risk: critical
