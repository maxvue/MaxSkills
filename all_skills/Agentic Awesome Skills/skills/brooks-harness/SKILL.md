---
name: brooks-harness
description: "Maintenance orchestrator for the brooks-lint plugin itself. Runs a sequential subagent pipeline — author → eval → QA → trigger-audit → release — to add or edit a skill, refresh the eval suite, keep the four manifests + README + CHANGELOG + AGENTS/GEMINI in sync, audit trigger... Use when developing, optimizing, debugging, or configuring brooks harness in production workflows."
risk: critical
