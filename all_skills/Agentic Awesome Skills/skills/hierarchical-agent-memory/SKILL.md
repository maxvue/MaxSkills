...
description: "Scoped CLAUDE.md memory system that reduces context token spend. Creates directory-level context files, tracks savings via dashboard, and routes agents to the right sub-context. Use when developing, optimizing, debugging, or configuring hierarchical agent memory in production workflows."
risk: safe
