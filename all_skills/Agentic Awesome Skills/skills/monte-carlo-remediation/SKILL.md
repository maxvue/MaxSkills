...
description: "Investigate and remediate data quality alerts using Monte Carlo MCP tools. Runs root cause analysis, assesses blast radius, discovers available tools (MCP/CLI/API), proposes and executes fixes, or escalates with full context when uncertain. Use when developing, optimizing, debugging, or configuring monte carlo remediation in production workflows."
risk: critical
