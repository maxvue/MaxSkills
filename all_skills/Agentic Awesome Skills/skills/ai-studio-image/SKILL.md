...
Ou crie um arquivo `.env` em `./scripts/run.py`:
```
...
```bash
pip install -r ./scripts/run.py
```

## 3. Gerar Sua Primeira Imagem

```bash
python ./scripts/run.py --prompt "mulher jovem tomando cafe em cafeteria" --mode influencer --format square
```
...
```bash
python ./scripts/run.py --prompt "prompt do usuario" --mode influencer
```
