...
> **Veja também:** para pipelines de imagem avançados (presets de qualidade, WebP/AVIF, thumbnails responsivos, streaming), a skill dedicada é `adonisjs-image-optimization-sharp-best-practices`. Use esta seção apenas para o processamento básico acoplado ao upload.
...
> **Veja também:** esta skill cobre o upload **server-side** (o arquivo passa pelo backend AdonisJS). Para uploads **diretos do navegador para o S3** (o cliente envia direto ao bucket via URL pré-assinada, sem sobrecarregar o servidor), use `adonisjs-s3-presigned-urls-direct-upload-best-practices`.
