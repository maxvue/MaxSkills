...
> **Veja também:** a configuração canônica de rate limiting (middleware HTTP, throttle por rota, integração com o Shield) fica em `adonisjs-security-shield-rate-limiting-best-practices`. Esta seção cobre apenas o rate limiting programático apoiado no Redis para integrações de API.
