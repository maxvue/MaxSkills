...
- **Padrão Lucid ORM:** Utilize a query builder do Lucid ORM para as consultas e verificações de registros. O model declara as propriedades em camelCase (`solarCompanyId`, `urlHash`) e a `CamelCaseNamingStrategy` padrão do Lucid v6 as mapeia para colunas snake_case no banco. Prefira métodos baseados no model (`Model.query().where('urlHash', hash)` aceita o nome da propriedade do model) em vez de cravar nomes de coluna; só use o nome físico snake_case (`url_hash`, `solar_company_id`) em SQL bruto ou `db.from('social_media_news')`.
