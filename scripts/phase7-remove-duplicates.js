...
/**
 * Script de Execução da FASE 7: REMOÇÃO FINAL DE SKILLS DUPLICADAS PRETERIDAS
 */
...
const REMOVAL_PLAN_PATH = path.join(ROOT_DIR, 'scripts', 'phase6_removal_plan.json')

function runPhase7() {
  console.log('🚀 Executando Fase 7: Remoção Final de Skills Duplicadas Preteridas...\n')
  ...
  for (const cat of allCatalogs) {
    ...
    for (const skill of cat.list) {
      if (removalIds.has(skill.id)) {
        ...
        // Limpar overlaps_ids das que foram removidas
        if (Array.isArray(skill.overlaps_ids)) {
          skill.overlaps_ids = skill.overlaps_ids.filter(id => !removalIds.has(id))
        }
  ...
  console.log(`✅ Fase 7 Concluída com Sucesso!`)
  console.log(`   - Total de Skills Duplicadas Removidas dos JSONs: ${totalRemoved}`)
