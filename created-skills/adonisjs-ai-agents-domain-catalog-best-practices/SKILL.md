...
### 4. Strategy Manager
- **Arquivos:** `strategy_manager.ts` / `strategy_manager_job.ts`. Classe `AgentInstagramStrategyManager`, `typeData: 'structured-data'`, modelo `gemini-2.5-flash`, `maxSteps: 30`, `maxCalls: 5`.
...
### 5. Event Replanner
- **Arquivos:** agente de replanejamento + job `ReplanEventJob`, fila `replan-event` (status `'replanning'`). Modelo `gemini-2.5-flash`, `maxSteps: 10`, `maxCalls: 2`.
...
- **Arquivos:** `protocol_extractor_agent.ts` / `analyze_protocol_job.ts`. Factory `createProtocolExtractorAgent(cardId: string): AgentConfig`. Modelo `gemini-2.5-flash` ou `gemini-2.5-flash-lite`, `temperature: 0`. Prompt com `<ENTRADA>`/`<TAREFA>`/`<REGRAS>`.
