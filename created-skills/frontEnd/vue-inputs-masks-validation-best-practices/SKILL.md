...
> **Prefira os componentes MaxComponentsUi já mascarados.** Em código de aplicação, para os casos cobertos use **sempre** os inputs dedicados do MaxComponentsUi em vez de `<input>` nativo + `v-maska`: `MaxInputCpfCnpj` (CPF/CNPJ), `MaxInputCep` (CEP), `MaxInputPhoneMail` (telefone/e-mail), `MaxInputNumber` (moeda/número). A diretiva `v-maska` em elemento nativo, mostrada abaixo, é a camada de baixo nível — use-a apenas para máscaras **ainda não cobertas** por um componente Max (ex.: placa de veículo), de preferência encapsulada num componente próprio, nunca como `<input>` solto numa página.
