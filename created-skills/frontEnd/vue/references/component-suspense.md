...
## No engeapp: o `await` de topo resolve por rota Ziggy

Suspense espera por componentes com `async setup()` ou `await` de topo em `<script setup>`. No engeapp esse `await` resolve via `apiGetRoute` (nome de rota Ziggy) ou store MaxPinia — nunca `fetch('/api/...')` cru.
