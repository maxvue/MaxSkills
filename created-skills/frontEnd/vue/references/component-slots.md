...
> **Nota engeapp:** o exemplo abaixo de "extraia para um composable" usa `useMouse` como ilustração didática. No engeapp, `useMouse` já existe pronto em `@maxvue/max-use` (re-exportado do VueUse) — nunca reimplemente composables utilitários já disponíveis lá.
...
```vue
<!-- MousePosition.vue -->
<!-- No engeapp: useMouse já vem pronto de @maxvue/max-use, não reimplemente. -->
<script setup lang="ts">
import { useMouse } from '@maxvue/max-use'
