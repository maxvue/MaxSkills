...
Composables de domínio devem compor os primitivos utilitários já prontos de `@maxvue/max-use` — nunca reimplementar `useEventListener`/`useMouse`/`useMouseInElement` etc. (ver `core-new-apis.md`). Componha só a parte que é específica do domínio:

```ts
// composables/useCardHoverState.ts
import { ref, watch, type Ref } from 'vue'
import { useMouseInElement } from '@maxvue/max-use'

export function useCardHoverState(cardElement: Ref<HTMLElement | null>) {
  const { isOutside } = useMouseInElement(cardElement)
  const highlighted = ref(false)

  watch(isOutside, (outside) => {
    highlighted.value = !outside
  })
