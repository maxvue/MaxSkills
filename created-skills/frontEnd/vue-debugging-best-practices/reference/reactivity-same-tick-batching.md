...
// Method 2: Track history with a sync watcher
// NOTE: you cannot redefine a ref's `.value` accessor via Object.defineProperty
// (count.value is just a number — calling .call() on it throws TypeError).
// To observe EVERY change, use a { flush: 'sync' } watch as the logging effect.
const history = []
watch(count, (val) => history.push(val), { flush: 'sync' })
```
