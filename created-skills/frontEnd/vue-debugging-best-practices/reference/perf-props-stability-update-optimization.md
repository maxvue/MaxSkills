...
<Item v-for="(item, index) in items" :key="item.id" :index="index" :total="items.length" /> <!-- Changes when list changes -->

<!-- BAD: Passing entire selection set -->
<Item v-for="item in items" :key="item.id" :selected-ids="selectedIds" /> <!-- All items re-render on any selection -->

<!-- GOOD: Pre-compute the boolean -->
<Item v-for="item in items" :key="item.id" :selected="selectedIds.includes(item.id)" />
```
