...
- **Seletor de Agente:** Use `MaxInputSelect` vinculado ao agente selecionado da store. Carregue as listas do backend através dos endpoints do `useSocialMediaAgentStore`.
- **Input de Prompt do Sistema:** Use `MaxInputText` com atributos de área de texto (ex: multiline ou linhas personalizadas) ou uma área de texto (textarea) estilizada com as propriedades do `InputBase`.
- **Parâmetros do Modelo (Sliders):** Como o `MaxComponentsUi` não possui um componente slider nativo:
  - Renderize um `<input type="range" min="0" max="1" step="0.1" />` formatado com propriedades de thumb do UnoCSS.
  - Agrupe-o com um `MaxInputText` somente leitura ou um indicador numérico para feedback visual preciso.
- **Tokens Máximos (Max Tokens):** Vincule a um input numérico restringindo o valor mínimo a `1` e o máximo a `8192` (ou limite atual do modelo).
- **Toggle de Ferramentas:** Use caixas de seleção (checkboxes) padrão ou alternadores customizados envelopados dentro de grids de layout responsivos (`MaxGrid` ou `MaxGridCols`).
...
- **Comportamento de Rolagem Automática (Auto-scroll):** Dispare ações de rolagem para o final (scroll-to-bottom) dentro de um observador `watch` ou quando ocorrerem atualizações. Implemente usando `nextTick`:
  ...
  ```
- **Integração com SSE / Transmit:** Configure ouvintes (listeners) para receber chunks de streaming do backend. Acumule o texto pedaço por pedaço de forma reativa para simular digitação em tempo real.
    ...
    <splitpanes class="default-theme flex-1 overflow-hidden">
      ...
      <pane size="35" min-size="25" max-size="50" class="pane-sidebar flex flex-col bg-slate-950 p-4 border-r border-slate-800 overflow-y-auto">
        ...
        <MaxGrid class="gap-4 mt-4">
          ...
          <!-- Prompt do Sistema -->
          <div class="s100">
            <label class="block text-xs font-semibold uppercase text-slate-400 mb-1">Instruções do Sistema</label>
            <textarea v-model="systemPrompt" rows="4" class="w-full bg-slate-900 border border-slate-800 rounded p-2 text-sm text-slate-200 focus:outline-none focus:border-orange-500" placeholder="Ex: Você é um copywriter especialista..." />
          </div>
          ...
          <div class="s50 flex items-center justify-start pt-6">
            <label class="flex items-center gap-2 cursor-pointer text-sm text-slate-300">
              <input v-model="enableTools" type="checkbox" class="accent-orange-500 h-4 w-4" />
              <span>Habilitar Tools</span>
            </label>
          </div>
        </MaxGrid>
      </pane>
      ...
      <pane size="65" class="flex flex-col bg-slate-900 overflow-hidden">
        ...
        <div class="p-4 border-t border-slate-800 bg-slate-950 flex gap-2">
          <input v-model="inputText" @keydown.enter.prevent="sendMessage" type="text" class="flex-1 bg-slate-900 border border-slate-800 rounded px-4 py-2 text-sm text-slate-200 focus:outline-none focus:border-orange-500" placeholder="Digite uma mensagem para o agente..." />
...
// Simulação de opções de agentes
const agentOptions = computed(() => [
  { value: 'copywriter', label: 'Copywriter Agent' },
  { value: 'strategy_manager', label: 'Strategy Manager Agent' },
  { value: 'editor', label: 'Editor Agent' }
]);
...
const sendMessage = () => {
  ...
  // Simulação de resposta em stream vinda do backend
  simulateStreamResponse();
};

// Simulação de comportamento de stream (SSE)
const simulateStreamResponse = () => {
    ...
    tools: [
      {
        ...
        output: '{"brandName": "MaxSolar", "niche": "Energia Solar"}'
      }
    ]
  };
  ...
  const fullText = 'Olá! Recebi sua mensagem de teste e o agente de IA está funcionando com os parâmetros configurados.';
  let index = 0;
