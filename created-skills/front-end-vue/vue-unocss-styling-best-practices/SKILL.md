...
- Mantenha a declaração limpa e reativa.
  - **Bom:** `<div class="p-4 rounded-md border" :class="{ 'border-emerald-500 bg-emerald-50': isActive, 'border-gray-200 bg-white': !isActive }">`
...
- Organize os blocos do componente SFC sempre na ordem obrigatória: `<template>`, `<script lang="ts">`, `<style lang="scss">`.
...
- **Não escreva cores hexadecimais manualmente** no HTML ou em CSS inline. Utilize sempre as classes de cores do tema do `presetMaxUno` (`emerald-X`, `red-X`, `blue-X`, etc.).
