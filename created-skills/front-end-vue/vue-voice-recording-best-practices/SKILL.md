...
## Objetivo
Fornecer diretrizes sólidas, padrões e restrições de segurança para a implementação de recursos de gravação de áudio e voz reativos, performáticos e livres de vazamentos de memória no Vue 3, utilizando a API MediaRecorder, AudioContext ou a biblioteca `vue-voice-recording`.
...
- Solicite acesso ao microfone do usuário usando `useUserMedia` do VueUse ou `navigator.mediaDevices.getUserMedia` nativo.
