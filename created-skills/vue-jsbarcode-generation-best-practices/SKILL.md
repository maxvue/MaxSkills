...
* Referencie os elementos do DOM usando `ref` com tipagem explícita do elemento SVG:
  ```typescript
...
* Prefira blocos SCSS escopados (`scoped`) para o estilo estático do componente. A única exceção é a transformação de escala dinâmica do zoom, que deve ser aplicada via `:style` reativo (ex.: `transform: scale(...)`) por depender de cálculo em tempo de execução.
