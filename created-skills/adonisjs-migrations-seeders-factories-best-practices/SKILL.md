...
* **Locale do Faker**: No `@faker-js/faker` v8+ a propriedade `faker.locale` foi removida. Para gerar dados em português do Brasil (ex: CNPJ, números de telefone, endereços), importe a instância localizada `fakerPT_BR` de `@faker-js/faker` em vez de atribuir `faker.locale`.
* **Relacionamentos**: Use o construtor `.relation()` para gerenciar relacionamentos de modelos durante a geração de dados simulados em testes/seeding. O nome passado a `.relation()` deve corresponder exatamente ao nome do relacionamento declarado no model (ex: `solarCompany`, não a coluna `solar_company_id`).
...
import User from '#models/user'
import { SolarCompanyFactory } from './solar_company_factory.js'

export const UserFactory = Factory
  .define(User, () => {
    // Usa a instância localizada pt_BR (faker.locale foi removido no v8+)
    return {
  ...
  // O nome deve bater com o relacionamento declarado no model User
  .relation('solarCompany', () => SolarCompanyFactory)
  .build()
```
