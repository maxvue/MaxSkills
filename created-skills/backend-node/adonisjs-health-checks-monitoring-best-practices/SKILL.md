...
export async function checkRedis() {
  ...
  try {
    ...
    // Sempre desconectar adequadamente para evitar vazamento de conexões de socket
...
## 4. Disponibilidade de APIs Externas de Redes Sociais (Meta / WhatsApp)
Verifique a conectividade com integrações externas (por exemplo, API do Graph do Meta, API do WhatsApp). Defina timeouts agressivos (máximo de 2 segundos) para evitar que a lentidão externa bloqueie a verificação de saúde interna.
...
export async function checkMetaApi() {
  const start = process.hrtime()
  try {
    // Pingar endpoint de status público ou fazer uma requisição HEAD para o endpoint base do Graph
    await axios.get('https://graph.facebook.com/status', { timeout: 2000 })
    const diff = process.hrtime(start)
    const latencyMs = Number((diff[0] * 1e3 + diff[1] * 1e-6).toFixed(2))
    ...
    return {
      status: 'WARNING',
      error: `Meta API inacessível: ${error.message}`,
    }
  }
}
```
...
- **Status Tolerante para Terceiros**: APIs de terceiros degradadas (Meta, WhatsApp) devem acionar um status `WARNING` em vez de `CRITICAL` para que ferramentas de orquestração de contêineres (Docker, Kubernetes) não executem reinicializações desnecessárias de contêineres.
