...
## Objetivo
Estabelecer padrões seguros, robustos e consistentes para a implementação de Autenticação de Dois Fatores (2FA) baseada em TOTP no backend AdonisJS v6 e no frontend Vue 3 dentro do ecossistema SocialMediaApp.
...
* **Design do Componente SFC (Composition API):**
  ...
  - Utilize os componentes do design system local `MaxComponentsUi` (`<MaxCard>`, `<MaxInputText>`, `<MaxButton>`, etc.).
