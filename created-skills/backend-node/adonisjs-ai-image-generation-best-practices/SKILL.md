...
export class ImageGenerationService {
   ...
   */
  async generate(prompt: string, size: '1:1' | '3:4' | '4:3' | '16:9' = '1:1'): Promise<Buffer> {
...
## 2. Engenharia de Prompts Visuais e Persona da Marca
A geração de imagens por IA produz melhores resultados quando os prompts originais são envelopados com predefinições de marca e melhorias estruturais.
...
- **Envelopamento de Prompt do Graphic Editor:**
```typescript
export function buildVisualPrompt(userPrompt: string, brandStyle: string): string {
  return `
    <StylePreset>
      Style: ${brandStyle}
...
export default class ImageProcessor {
  ...
  async processAndStore(prompt: string, agencyId: string): Promise<string> {
    // 1. Gera o Buffer da imagem
    const imageBuffer = await this.generator.generate(prompt)

    // 2. Define o caminho da chave única
    const key = `agencies/${agencyId}/generated-images/${cuid()}.jpg`
...
export default class ImageGenerationJob {
  ...
  static async handle(job: Job<ImageGenerationJobData>) {
    const { eventId, prompt, agencyId } = job.data
    const processor = new ImageProcessor()

    try {
      const fileKey = await processor.processAndStore(prompt, agencyId)

      // Persiste o resultado no banco de dados e atualiza o registro do Evento/Post
      // Exemplo:
      // const event = await Event.findOrFail(eventId)
      // await event.merge({ imageUrl: fileKey, status: 'ready' }).save()

      // Registra o Custo da API
      await saveAiCost('Event', eventId, {
