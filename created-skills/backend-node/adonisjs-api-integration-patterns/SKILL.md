...
* **Serviços Dedicados**: Encapsule todas as comunicações de APIs externas dentro de classes de Serviço dedicadas. Crie-as sob `app/services/` (ex: `app/services/payment_service.ts`, `app/services/meta_api_service.ts`).
...
* Se uma operação de API for lenta ou não exigir resposta imediata ao usuário (ex: exportação de arquivos, sincronização de contatos, publicação de mídias), envie a tarefa para uma fila em background usando o BullMQ.
...
```typescript
// app/services/meta_api_service.ts
...
export default class MetaApiService {
  private baseUrl = env.get('META_API_BASE_URL')
  private accessToken = env.get('META_API_ACCESS_TOKEN')
  ...
  /**
   * Publica uma mídia no Instagram
   */
  async publishMedia(
    instagramAccountId: string,
    payload: InstagramPublishPayload
  ): Promise<InstagramPublishResponse> {
    const url = `${this.baseUrl}/${instagramAccountId}/media`
    const timeoutMs = 8000 // 8 segundos

    this.logger.debug({ url: this.redact(url), payload }, 'Enviando requisição para a API do Meta')
    ...
    try {
      ...
      this.logger.debug(
        ...
        'Resposta recebida da API do Meta'
      )

      if (!response.ok) {
        throw new ExternalApiException(
          `A requisição à API do Meta falhou com status ${response.status}: ${responseText}`,
          { status: response.status }
        )
      }

      return JSON.parse(responseText) as InstagramPublishResponse
    } catch (error: any) {
      this.logger.error(
        { 
          error: error.message, 
        ...
        'Erro ao se comunicar com a API do Meta'
      )

      if (error.name === 'TimeoutError') {
        throw new ExternalApiException('A requisição à API do Meta expirou (timeout)', { status: 504 })
      }
      ...
      throw new ExternalApiException(`Erro de comunicação com a API do Meta: ${error.message}`, {
        status: 500,
        cause: error,
      })
    }
