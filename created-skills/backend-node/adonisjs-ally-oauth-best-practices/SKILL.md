...
description: Use when configuring, implementing, or debugging social login (OAuth2) with AdonisJS Ally para os provedores Google e Facebook, com rotas /auth/:provider/redirect e /auth/:provider/callback, tratamento de accessDenied/hasError, find-or-create de usuário por email e login via session guard web. Triggers on Ally config, callback handling e fluxo de login social.
---
...
* Exponha `GET /auth/providers` para o frontend descobrir quais provedores estão configurados, baseando-se na presença das variáveis de ambiente. Só lista o provedor cujo `*_CLIENT_ID` está definido:
  ...
  async providers({ response }: HttpContext) {
      ...
      const list: string[] = [];
      if (env.default.get('GOOGLE_CLIENT_ID')) list.push('google');
      if (env.default.get('FACEBOOK_CLIENT_ID')) list.push('facebook');
...
* **Vinculação por email**: o email retornado pelo provedor é a chave de identidade. Procure um usuário existente por email; se não existir, crie-o. Isso vincula automaticamente logins de Google e Facebook que compartilham o mesmo email à mesma conta:
  ```typescript
...
* **MANTENHA** os redirects de erro exatos do fluxo: `/login?error=access_denied`, `/login?error=oauth_error`, `/login?error=no_email`; e o sucesso em `/projects`.
