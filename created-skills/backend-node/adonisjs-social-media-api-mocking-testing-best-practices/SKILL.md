...
1. **Mocking de Requisições HTTP Externas**:
   ...
   - Use `nock` ou interceptores do Axios para interceptar e mockar requisições de rede de saída.
...
## Exemplos
### Mocking de Chamadas Axios usando Nock
```typescript
...
- Não faça mock de toda a interface da classe se estiver realizando testes de integração; em vez disso, simule na camada de transporte HTTP usando nock ou interceptores do Axios.
