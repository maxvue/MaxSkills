...
* **Timezone do Cliente/Tenant:** Salve o nome do fuso horário IANA preferencial (ex: `'America/Sao_Paulo'`, `'America/Manaus'`) como uma coluna de texto (ex: `timezone`) na tabela de configurações do cliente/tenant (ex: `MarketingAgency` ou `SolarCompany`).
