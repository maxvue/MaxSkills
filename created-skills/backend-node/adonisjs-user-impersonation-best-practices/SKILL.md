...
*   **Payload / Cabeçalhos da Resposta:** Quando o frontend solicitar dados do usuário (ex: `/user/data`), retorne uma flag booleana adicional `isImpersonated` e os detalhes do personificador, se ativos.
*   **Integração com Vue:** Garanta que as requisições do frontend verifiquem esta flag para exibir uma barra de aviso persistente e visível (ex: "Você está visualizando esta conta como [Nome do Usuário]. Clique aqui para retornar ao Painel Admin.").
