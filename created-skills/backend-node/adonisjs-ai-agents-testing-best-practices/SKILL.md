...
## 2. Configurando Testes Unitários com MockLanguageModelV3
Use a classe `MockLanguageModelV3` de `ai/test` para interceptar as chamadas e retornar respostas determinísticas:
1. Importe `MockLanguageModelV3` no seu arquivo de teste `.spec.ts`.
...
* **SEM Segredos Hardcoded:** Nunca inclua chaves de API reais ou credenciais do Soketi nos arquivos de teste; utilize variáveis de ambiente ou mocks.
