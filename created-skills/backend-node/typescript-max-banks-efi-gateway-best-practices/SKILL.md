...
* Para pagamentos com cartão de crédito e assinaturas recorrentes:
  ...
  - Interaja com os endpoints `/v1/subscription` para gerenciar planos ativos e ciclos de cobrança recorrentes do cliente.
  - Traduza os estados de assinatura da Efí para estados canônicos: `ATIVA` é mapeado para `active`, `INADIMPLENTE` é mapeado para `past_due` e `CANCELADA` é mapeado para `canceled`.
