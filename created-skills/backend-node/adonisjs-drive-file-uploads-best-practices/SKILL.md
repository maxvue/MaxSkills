...
export default class AvatarController {
  async update({ request, response, auth }: HttpContext) {
    ...
    await user.merge({ avatarUrl: key }).save()
...
- **Ativos Públicos:** Se o disco estiver configurado como `visibility: 'public'`, resolva as URLs usando `drive.use().getUrl(key)`.
- **Ativos Privados/Protegidos:** Se os arquivos só devem ser acessados por usuários autorizados (ex: faturas, contratos), configure o armazenamento privado e gere URLs assinadas usando `drive.use().getSignedUrl(key, { expiresIn: '30m' })`.

```typescript
import drive from '@adonisjs/drive/services/main'

// URL Pública
const publicUrl = await drive.use().getUrl('avatars/user-1.webp')
