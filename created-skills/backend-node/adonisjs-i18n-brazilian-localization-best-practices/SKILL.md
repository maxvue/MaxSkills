...
* `resources/lang/pt-BR/validator.json`
```json
{
  "shared": {
...
Para detectar automaticamente o locale do cliente (a partir de cookies, sessão ou do cabeçalho `Accept-Language`), certifique-se de que o middleware nativo `DetectUserLocaleMiddleware` está registrado no arquivo `start/kernel.ts`.
```typescript
// start/kernel.ts
router.use([
...
Use as funções de formatação da instância de `i18n` para exibir consistentemente datas e moedas no formato brasileiro (BRL / pt-BR) tanto no backend quanto nos templates do Edge.
...
#### Templates Edge
```html
<p>Total: {{ i18n.formatCurrency(transaction.amount, { currency: 'BRL' }) }}</p>
<p>Data: {{ i18n.formatDate(transaction.createdAt, { dateStyle: 'medium' }) }}</p>
```
