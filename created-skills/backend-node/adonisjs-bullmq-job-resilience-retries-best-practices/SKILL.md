...
- Para **erros permanentes**: Capture o erro e chame `await job.discard()` antes de relançá-lo ou encerrar o fluxo para evitar tentativas desnecessárias.
  ...
  static async handle(job: Job<FetchNewsJobData>) {
    try {
      ...
      if (error.status === 404 || error.code === 'E_VALIDATION_ERROR') {
        // Erro permanente: descarta tentativas adicionais
        await job.discard()
...
Exemplo de estrutura dentro de `commands/worker.ts`:
```typescript
