...
* **Backend: Definição e Inferência de Tipo**
  Defina e compile o validador em seu controller ou arquivo de validadores, e exporte seu tipo inferido usando `Infer`:
  ```typescript
...
* **Frontend: Importando e vinculando a formulários**
  Importe o tipo do validador para tipar estados reativos em componentes Vue:
  ```typescript
  import type { SavePayload } from '#controllers/brand_positioning_controller'
...
### 4. Estruturando Requisições e Respostas com Tipagem Estrita
Garanta que os wrappers de requisições de API (clientes Axios ou Fetch) sejam tipados com os payloads e respostas exatos do backend.

> **Atenção ao contrato MaxPinia.** No padrão do projeto, dados de página são lidos/salvos via store `@maxvue/max-pinia`: o GET de dados retorna `{ data: ... }` e o save retorna `{ success: true }` (ver `adonisjs-maxpinia-endpoint-patterns`). Tipar a resposta de um save como o model cru (`Promise<SolarCompany>` abaixo) só é correto para um endpoint REST avulso fora do fluxo de store. Para um endpoint de save de store, o tipo de resposta é o envelope, p.ex. `Promise<{ success: true }>`.
...
// Endpoint REST avulso (NÃO backing de store MaxPinia):
export async function updateCompany(data: SavePayload): Promise<SolarCompany> {
  const response = await axios.post<SolarCompany>('/api/company', data)
