...
* Utilize o `fetch` nativo do Node.js ou um wrapper HTTP confiável (como `Axios`) para fazer a requisição POST para a URL do webhook do Slack.
...
* Estruture alertas e logs utilizando o formato JSON **Block Kit** do Slack para tornar as notificações legíveis:
  ...
  * **Actions Block (Ações):** Forneça botões interativos com links para o Sentry, Telescope ou para a página de detalhes do erro.
