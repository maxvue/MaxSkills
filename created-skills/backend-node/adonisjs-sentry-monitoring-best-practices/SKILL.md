...
Adicione o DSN do Sentry e as variáveis de ambiente de configuração no arquivo `#start/env` e `start/env.ts`:
```typescript
// start/env.ts
...
## 2. Service Provider Dedicado para Inicialização do Sentry
Para garantir que o Sentry instrumente os pacotes core (como módulos de HTTP e banco de dados) antes de serem inicializados, crie um Service Provider:
```bash
node ace make:provider sentry --no-interaction
```

Abra `providers/sentry_provider.ts` e inicialize o Sentry durante a fase de `register`:
```typescript
...
Garanta que o provider está registrado em `adonisrc.ts` dentro do array `providers`:
```typescript
providers: [
  // ... outros providers
  () => import('./providers/sentry_provider.js'),
]
```
...
- **Inicialização Antecipada:** Certifique-se de que o SDK do Sentry seja inicializado na fase `register` do provider, antes do roteamento ou inicialização de qualquer conexão com banco de dados.
