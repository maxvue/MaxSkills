...
### Documentação TypeScript (JSDoc/TSDoc + TypeDoc)

Para gerar documentação inline e referência de API a partir de código TypeScript, use JSDoc/TSDoc nas APIs públicas e o TypeDoc para a geração.

**Tags JSDoc/TSDoc mais usadas:**
...
| `@deprecated` | Marcar APIs depreciadas (com orientação de migração) |

**Boas práticas de JSDoc:**

1. Documente todas as APIs públicas (métodos, classes, interfaces, tipos exportados)
2. Use `@example` com código executável para funções complexas
3. Inclua `@throws` para todos os erros possíveis
4. Use `@see` para referência cruzada de funções/tipos relacionados
5. Documente restrições de generics e parâmetros de tipo
6. Adicione avisos de segurança quando relevante — nunca inclua segredos/credenciais
...
- [ ] Todos os `@param` têm descrição
- [ ] Todos os `@returns` têm descrição
...
\```vue
<template>
  <NomeDoComponente titulo="Meu Título" :itens="listaDeItens" @selecionar="aoSelecionar" />
</template>
 ...
 *
 * @remarks
 * Desempenho: O(n) onde n é o comprimento da string do e-mail.
 *
