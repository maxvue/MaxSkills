...
test.describe('Proposta Fotovoltaica - Assistente de IA', () => {
  test('deve exibir mensagem de erro amigável quando o endpoint de IA falhar', async ({ page }) => {
    ...
    await expect(errorAlert).toContainText('AI service is temporarily unavailable');
  });
});
```

### 5. Interações Ricas: Arrastar e Soltar no Cronograma
Simule movimentos complexos de mouse, como arrastar etapas de instalação entre células do cronograma (ex: FullCalendar).
```typescript
test('deve arrastar e soltar uma etapa de instalação para reagendá-la', async ({ page }) => {
  await page.goto('/schedule');

  const sourceEvent = page.locator('.fc-event:has-text("Instalação de painéis")').first();
  ...
  await expect(toast).toContainText('Etapa reagendada com sucesso');
});
```
...
```typescript
test('a prévia da proposta deve renderizar corretamente', async ({ page }) => {
  await page.goto('/proposals/1');
  
  // Aguarda que os gráficos de geração/economia sejam carregados
  await page.waitForSelector('[data-testid="proposal-preview"] canvas');
  
  // Captura o screenshot e compara com o snapshot de referência
  await expect(page.locator('[data-testid="proposal-preview-container"]')).toHaveScreenshot('proposal-preview-baseline.png');
});
```
