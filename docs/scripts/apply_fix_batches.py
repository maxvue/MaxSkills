#!/usr/bin/env python3
"""
Aplica as correções semânticas dos 12 lotes auditados aos arquivos SKILL.md.
"""
import glob
import json
import os
...
def main():
    results = sorted(glob.glob('docs/reports/fix_batches/result_*.json'))
    print(f"Encontrados {len(results)} arquivos de resultados.")
    ...
    if errors:
        for err in errors:
            print("  ERRO:", err)
        raise SystemExit(1)
    print("Aplicação concluída com 100% de integridade!")
